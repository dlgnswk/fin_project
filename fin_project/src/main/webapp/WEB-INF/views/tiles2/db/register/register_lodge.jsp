...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("select.start_time").val("1").trigger("change"); // start_time의 첫번째 값 선택 이벤트
...
//			console.log($(this).val());
		...
		if( "${requestScope.lovo.lodge_id}" != "" &&  "${requestScope.lovo.lodge_id}" != null) {
			...
			if(fd_status.val() == 1) {
				...
				y_btn_after.find("input[name='fd_time']").val("${requestScope.lovo.fd_time}");
				
				
				if("${requestScope.lovo.fd_time}".indexOf("24시간") != -1){
					y_btn_after.find(".all_time_").prop("checked",true);
					y_btn_after.find("select").prop("disabled", true);
				}
				else {
					let fd_time= "${requestScope.lovo.fd_time}".split(" ~ ");
					
					let up_start_time = '';
					if(fd_time[0].indexOf("AM") >= 0) {
						up_start_time = Number(fd_time[0].split(":")[0]);
					} 
					else if(fd_time[0].indexOf("PM") >= 0){
						up_start_time = Number(fd_time[0].split(":")[0])+12;
					} 
					else if( fd_time[0].indexOf("정오") >=0 ) {
						up_start_time = 12;
					} 
					else if( fd_time[0].indexOf("자정") >=0 ) {
						up_start_time = 24;
					}
					
					y_btn_after.find("select.start_time").val(up_start_time);
					y_btn_after.find("select.end_time").val(fd_time[1]);				
				}
			...
			let lg_checkin_start_time = "${requestScope.lovo.lg_checkin_start_time}";
			let checkIn_startTime = "";
			if(lg_checkin_start_time.indexOf("AM") >= 0) {
				checkIn_startTime = Number(lg_checkin_start_time.split(":")[0]);
			} 
			else if(lg_checkin_start_time.indexOf("PM") >= 0){
				checkIn_startTime = Number(lg_checkin_start_time.split(":")[0])+12;
			} 
			else if( lg_checkin_start_time.indexOf("정오") >=0 ) {
				checkIn_startTime = 12;
			} 
			else if( lg_checkin_start_time.indexOf("자정") >=0 ) {
				checkIn_startTime = 24;
			}
			$("input[name='lg_checkin_start_time']").closest(".__data").find(".start_time").val(checkIn_startTime);
			$("input[name='lg_checkin_start_time']").val("${requestScope.lovo.lg_checkin_start_time}");
			
			$("select[name='lg_checkin_end_time']").val("${requestScope.lovo.lg_checkin_end_time}");
				...
				$.each(json, function(index, item){
					...
					$.each(checkbox_text, function(idx, elmt){
						if(item.fk_pool_opt_no == $(elmt).val()) {
							$(elmt).prop("checked", true);
							$(elmt).parent('.checkbox_div').find(".use_time").show();
							let pl_use_time = item.pool_use_time; 
							let checkbox_div = $(elmt).parent('.checkbox_div');
							
							if(pl_use_time.indexOf("24시간") != -1){
								checkbox_div.find(".all_time_").prop("checked",true);
								checkbox_div.find("select").prop("disabled", true);
							}
							else {
								let use_time = pl_use_time.split(" ~ ");
								
								let up_start_time = '';
								if(use_time[0].indexOf("AM") >= 0) {
									up_start_time = Number(use_time[0].split(":")[0]);
								} 
								else if(use_time[0].indexOf("PM") >= 0){
									up_start_time = Number(use_time[0].split(":")[0])+12;
								} 
								else if( use_time[0].indexOf("정오") >=0 ) {
									up_start_time = 12;
								} 
								else if( use_time[0].indexOf("자정") >=0 ) {
									up_start_time = 24;
								}
								checkbox_div.find("select.start_time").val(up_start_time);
								checkbox_div.find("select.end_time").val(use_time[1]);
							}
							checkbox_div.find("input[name='pool_use_time']").val(pl_use_time).prop("disabled",false);
						}
					}); // end of $.each(checkbox_text, function(idx, elmt)
	...
	//	console.log(submit_val);
		...
		// "아니오" 
			...
			e_par.parent().next("div.y_btn_after").find("input:text").val("");
			e_par.parent().next("div.y_btn_after").find("input[name='input:text']").val("");
	...
	function func_geocoder(address) {
 	...
 	// 시작시간을 선택한다 -> 끝나는 시간을 select 문에 보여준다 -- 시간 선택시 발생하는 함수
	function func_select_time(e) {
		
		let end_tiem_html = "";
		
		end_tiem_html += "<option value=''>--선택--</option>";
		
		if( $(e).hasClass("anytime__") ) {
			end_tiem_html += "<option>언제든지</option>";
		}
		
		let time = Number($(e).val()) + 1;
		
		// == 시작 시간에 따른 끝나는 시간 만들기 -시작- == //
		for(let i=time; i<time+23; i++) {
						
			if( i <= 12 ) {
				if( i == 12) {
					end_tiem_html += `<option>정오</option>`;
				}
				else {
					if( i < 10) {
						end_tiem_html += `<option>0\${i}:00 AM</option>`;
					}
					else {
						end_tiem_html += `<option>\${i}:00 AM</option>`;
					}
				}
			}
			else if (i > 12) {
				if(i == 24) {
					end_tiem_html += `<option>자정</option>`;
				}
				else if (i < 24){
					if( i-12 < 10) {
						end_tiem_html += `<option>0\${i-12}:00 PM</option>`;
					}
					else{
						end_tiem_html += `<option>\${i-12}:00 PM</option>`;
					}
				}
				else if (i > 24) {
					if( i-24 <= 12 ) {
						if( i-24 == 12) {
							end_tiem_html += `<option>다음날 정오</option>`;
						}
						else {
							if( i-24 < 10) {
								end_tiem_html += `<option>다음날 0\${i-24}:00 AM</option>`;
							}
							else {
								end_tiem_html += `<option>다음날 \${i-24}:00 AM</option>`;
							}
						}
					}
					else if (i-24 > 12) {
						if(i == 24) {
							end_tiem_html += `<option>다음날 자정</option>`;
						}
						else if (i-24 < 24){
							if( i-24-12 < 10) {
								end_tiem_html += `<option>다음날 0\${i-24-12}:00 PM</option>`;
							}
							else{
								end_tiem_html += `<option>다음날 \${i-24-12}:00 PM</option>`;
							}
						}
					}
				}
		...
		// == 시작 시간에 따른 끝나는 시간 만들기 -끝- == //
		
		$(e).parent().find(".end_time").html(end_tiem_html);
		
		
		let start_time = $(e).closest(".__data").find(".start_time option:selected").text();
		let end_time = $(e).closest(".__data").find(".end_time option:selected").text();
		
		$(e).closest(".__data").find("input[name='fd_time']").val(`\${start_time} ~ \${end_time}`);
		$(e).closest(".__data").find("input[name='lg_checkin_start_time']").val(`\${start_time}`);
		
		let start_time_2 = $(e).closest(".use_time").find(".start_time option:selected").text();
		let end_time_2 = $(e).closest(".use_time").find(".end_time option:selected").text();
		$(e).closest(".use_time").find("input[name='pool_use_time']").val(`\${start_time_2} ~ \${end_time_2}`);		
	...
	// 끝나는 시간 선택 -시작-
	function func_select_end_time(e) {
				...
				<form name="lodge_info_Frm">
					...
					<div class="__data y_btn_after">
						...
						<div class="fd_time">
							...
							&nbsp;~&nbsp;
							<select class="end_time" onchange="func_select_end_time(this)">
							</select>
						</div>
					...
					<div class="__data">
						...
						<div class="lg_checkin_time_div">		
							...
							&nbsp;~&nbsp;
							<select class="end_time" name="lg_checkin_end_time" onchange="func_select_end_time(this)">
							</select>
						</div>
					</div>
